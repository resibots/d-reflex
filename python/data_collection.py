...
while len(good_samples)<N:
    ...
    for name in os.listdir(path):
        jobs.append((loader, {"path": path, "name":name, "files":["real_q.dat", "tsid_q.dat", "end.dat", "config"]}))
    n_jobs = len(jobs)
    ...
    for dic in dicts.values():
        ...
        key = (rh[0],rh[1],rh[2],lh[0],lh[1],lh[2], wall_angle, wall_distance)
        if dic["stop_reason"]['Nothing_Default'][0] == "Success" and 'tsid_q' in dic["data"]["Nothing_Default"]:
            good_samples.append({"key": key, "tsid_q": dic["data"]["Nothing_Default"]['tsid_q'], "real_q": dic["data"]["Nothing_Default"]['real_q']})
...
for dic in good_samples[:N]:
    ...
    random_params[dic['key']] = {"wall_angle": wall_angle, "wall_distance": wall_distance, "lh": list(lh), "rh": list(rh), "q": dic['tsid_q'], "starting_configuration": dic['real_q']}

# ## Do I fall

conditions = [[f"Passive_R_Leg_{i}" for i in range(4,7)], [f"Cut_R_Leg_4"]]
...
for param in random_params.values():
    ...
    fall_dicts[name]["conditions"] = [[Conditions["A"][c] for c in cs] for cs in conditions]
...
jobs = []
for name in os.listdir(path):
    jobs.append((loader, {"path": path, "name":name, "files":["end.dat", "config"]}))
n_jobs = len(jobs)
...
conditions_name = set()
for name, dic in tqdm(dicts.items()):
    config = dic["config"]
    lh = config['BEHAVIOR']["lh_shift"]
    rh = config['BEHAVIOR']["rh_shift"]
    wall_distance = config['CONTROLLER']['wall_distance']
    wall_angle = config['CONTROLLER']['wall_angle']
    key = (rh[0],rh[1],rh[2],lh[0],lh[1],lh[2], wall_angle, wall_distance)
    for k, name in enumerate(dic["stop_reason"].keys()):
        stop_reason = list(dic["stop_reason"].values())[k]
        random_params[key]["no_reflex_end"][name] = stop_reason[0] == "Success"
        conditions_name.add(name)

for condition in conditions_name:
    count = 0
    for param, dic in random_params.items():
        count += dic["no_reflex_end"][condition]
    print(f"{condition}: no reflex success rate {count*100/len(random_params):2.1f}%")
...
for param in random_params.values():
    ...
    for x in X:
        ...
        for z in Z:
            ...
            contact_dicts[name]["conditions"] = [[Conditions["A"][c] for c in cs] for cs in conditions]
...
jobs = []
for name in os.listdir(path):
    jobs.append((loader, {"path": path, "name":name, "files":["end.dat", "config", "wall_contact.dat"]}))
n_jobs = len(jobs)
...
## clean truth 
for dic in random_params.values():
    dic["truth"] = {key: np.zeros((len(Z), len(X))) for key in conditions_name}
    dic["time"] = {key: np.zeros((len(Z), len(X))) for key in conditions_name}
...
for name, dic in tqdm(dicts.items()):
    ...
    for k, name in enumerate(dic["stop_reason"].keys()):
        ...
        random_params[key]["time"][name][j][i] = stop_reason[1]
        random_params[key]["truth"][name][j][i] = int(stop_reason[0] == "Success")
...
for condition in conditions_name:
    count = 0
    for param, dic in random_params.items():
        count += np.sum(dic["truth"][condition])>0
    print(f"{condition}: success rate {count*100/len(random_params):2.1f}%")
