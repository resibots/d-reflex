...
# ## Example fall when we do nothing 

for xp in tqdm(range(20, min(120,len(params)))):
    ...
    videos_folder = "/home/pal/notebooks/data/wall_reflex/videos/default_fall/"
    shutil.move(video_path, videos_folder+f"{xp}_{pretty_print_condition(condition)}.mp4")

    cam = cv2.VideoCapture(videos_folder+f"{xp}_{pretty_print_condition(condition)}.mp4")
    ret, frame = cam.read()
    while ret:
        previous_frame = frame
        ret, frame = cam.read()
    name = videos_folder+f"{xp}_{pretty_print_condition(condition)}.png"
...
for xp in tqdm(range(20, min(120,len(params)))):
    ...
    videos_folder = "/home/pal/notebooks/data/wall_reflex/videos/examples_success/"
    shutil.move(video_path, videos_folder+f"{xp}_{pretty_print_condition(condition)}.mp4")
    cam = cv2.VideoCapture(videos_folder+f"{xp}_{pretty_print_condition(condition)}.mp4")
    ret, frame = cam.read()
    while ret:
    ...
    ax.pcolor(dic["map"], cmap="viridis", edgecolors='k', vmin=np.min(dic["map"]), vmax=np.max(dic["map"]), linewidths=1)
    ...
    plt.savefig(videos_folder+f"{xp}_{pretty_print_condition(condition)}_truth.png", pad_inches = 0)
    plt.close()
    fig, ax = plt.subplots(figsize=(d,d))
    ax.pcolor(dic['predict_map'], cmap="viridis", edgecolors='k', vmin=0, vmax=1, linewidths=1)
    ...
    plt.gca().xaxis.set_major_locator(plt.NullLocator())
    plt.gca().yaxis.set_major_locator(plt.NullLocator())
    plt.savefig(videos_folder+f"{xp}_{pretty_print_condition(condition)}_pred.png", pad_inches = 0)
    plt.close()
